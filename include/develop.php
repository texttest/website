...
<div class="Text_Normal">
The "default" configuration is used if <A class="Text_Link" HREF="index.php?page=documentation_trunk&n=writing_a_config_module#config_module">"config_module"</A> is not set and there is only 1 core on the local machine. It makes use of an ActionRunner object, which maintains a sequence of plugins.Action objects. These consist of actions to carry out in sequence on the tests that are loaded. The sequence is determined by the getActionSequence() method on the Configuration object. A typical test run contains separate Actions to make the sandbox, copy and link the relevant test data there, filter the approved files, run the test, filter the temporary files, compare the results and act on the results. 
</div>
<div class="Text_Normal">
The "queuesystem" configuration is used if <A class="Text_Link" HREF="index.php?page=documentation_trunk&n=writing_a_config_module#config_module">"config_module"</A> is not set and there is more than 1 core on the local machine. It has a secondary module-finding config setting, "queue_system_module", which points out which queuesystem set up (local, lsf, sge, condor, ec2cloud currently) is to be used. Note that this works via TextTest (the "master" process) submitting a copy of itself (the "slave" process) to the queue module concerned. These two processes then commmunicate with each other via the slave initiating socket connections and receiving responses from the master.
</div>
<div class="Text_Normal">
