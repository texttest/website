<div class="Text_Header">Maintainable GUI testing with StoryText</div> 
...
<div class="Text_Normal">
  ...
  they understand: i.e. that of the domain. StoryText (formerly "PyUseCase") is
  a tool that can record and replay such sequences and thereby
...
<div class="Text_Normal">
  ...
  external file in StoryText, administered via a small GUI which prompts you
...
<div class="Text_Normal">
  So StoryText can record and replay usecases for us. But how
...
<div class="Text_Normal">
  ...
  what it does by monitoring the contents of that log using TextTest. StoryText does
  this for you: it generates an ASCII-art type log of the current GUI appearance and monitors
  changes to it. The application can supplement it with its own logging as it wishes. 
</div>
...
<div class="Text_Normal">
  StoryText introduces the concept of an <b>"Application Event"</b>. This
  is basically some instrumentation in the code that indicates to the tool
  that something has happened that needs to be waited for, thus allowing
...
<div class="Text_Header">Raising the abstraction level in tests</div>
<div class="Text_Normal">
  Ordinarily, you get one line in your usecase per click done in the UI. This is OK
  for simple UIs but can get unwieldy for complicated ones. Fortunately, StoryText has
  a mechanism whereby common elements can be extracted and saved as a so-called
  <b>"GUI shortcut"</b>. This mechanism is described in more detail 
